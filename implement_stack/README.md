...
Implementation of the "Last-In-First-Out" (LIFO) data structure, also known as a stack, using only two queues. The stack implementation should support all functions of a normal stack (push, top, pop, and empty).

Implement the class "MyStack":
...
You should only use standard queue operations, which means only operations like push back, peek/pop front, size, and is empty.
Depending on your language, the queue might not be natively supported. You can simulate a queue using a list or a deque (double-ended queue), as long as you only use standard queue operations.
My Thoughts
The goal of the exercise is to replicate the Last-In, First-Out (LIFO) algorithm—Last In, First Out. It is necessary to use several functions to simulate this algorithm, such as pop (which removes the element at the top of the stack and returns it), push (which adds an element to the top of the stack), top (which returns the element above the stack), and empty (which returns true if the stack is empty).
...
The algorithm implements a data structure called a stack using a dynamic array. The stack follows the Last-In, First-Out (LIFO) principle, meaning the last element added is the first to be removed. Here's a detailed description of each function:

my_stack_create():

Time Complexity: O(1)
Term: Static memory allocation for the my_stack structure and the items array. No additional allocation after creation.
is_full(my_stack obj):

Time Complexity: O(1)
Term: No additional allocation. Checking if the stack is full.
my_stack_push(my_stack obj, int x):

Time Complexity: O(1)
Term: No additional allocation. The items array has been preallocated with a fixed size.
my_stack_pop(my_stack obj):

Time Complexity: O(1)
Term: No additional allocation. Memory space for elements is already allocated.
my_stack_top(my_stack obj):

Time Complexity: O(1)
Term: No additional allocation. Direct access via the top index.
my_stack_empty(my_stack obj):

Time Complexity: O(1)
Term: No additional allocation. Comparison operation without using additional space.
my_stack_free(my_stack obj):

Time Complexity: O(1)
Term: Freeing allocated memory for the items array and the my_stack structure. No additional allocation.
In summary, the spatial complexity is constant (O(1)) for each stack operation, as no additional dynamic memory allocation is performed as the stack is used.

## Complexity

### Temporal
Overall, the temporal complexity of each operation is constant (O(1)), meaning the execution time of each operation does not depend on the size of the stack. This makes the stack implementation very efficient for individual operations.

### Spatial
The spatial complexity is constant (O(1)) for each stack operation, as no additional dynamic memory allocation is performed as the stack is used.
